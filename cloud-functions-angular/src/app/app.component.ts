...
export class AppComponent {
  ...
  constructor(public db: AngularFireDatabase, public afAuth: AngularFireAuth, public snackBar: MatSnackBar) {
    ...
    this.user.subscribe((user: firebase.User) => {
      ...
      if (user) { // User is signed in!
        ...
        this.messages = this.db.list('/messages', {  // *** here is where load the existing messages
          query: {
            limitToLast: 12
          }
        });
        console.log("we just set the messages field");
  ...
  // TODO: Refactor into text message form component
  saveMessage(event: any, el: HTMLInputElement) {      // *** here is the saveMessage() function use dto push to firebase
  ...
  saveImageMessage(event: any) {  // *** function for saving images
    event.preventDefault();
